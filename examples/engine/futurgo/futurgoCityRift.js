...
FuturgoCity.prototype.go = function() {
	...
	// Create the viewer
	this.viewer = new glam.Viewer({ container : this.container, 
		firstPerson:true,
		showGrid:false,
...
FuturgoCity.prototype.addBackground = function() {
	...
	var skybox = glam.Prefabs.Skybox();
	var skyboxScript = skybox.getComponent(glam.SkyboxScript);
...
FuturgoCity.prototype.addCollisionBox = function() {
	...
	var bbox = glam.SceneUtils.computeBoundingBox(this.scene);

	var box = new glam.Object;
...
FuturgoCity.prototype.onFuturgoLoadComplete = function(data) {
	...
	futurgoScene.map(/windows_front|windows_rear/, function(o) {
		...
		var fader = new glam.FadeBehavior({duration:2, 
	...
	this.pickers = [];
	futurgoScene.map("glam_mobile", function(o) {
	...
	futurgoScene.map(glam.PointLight, function(light) {
		light.intensity = 0;
	});
	...
	var futurgo = futurgoScene.findNode("glam_mobile");
	futurgo.transform.position.set(2.33, 0, -6);

	// Drop a camera inside the vehicle
	var driveCam = new glam.Object;
	var camera = new glam.PerspectiveCamera({fov:75});
	camera.near = 0.01;
	...
	this.hud = glam.Prefabs.HUD({zDistance:3});
...
//		that.helpScreenVisible = true;
	...
	if (glam.Gamepad && glam.Gamepad.instance) {
...
FuturgoCity.prototype.onGamepadButtonsChanged = function(event) {
	...
	for (i = 0; i < len; i++) {
		...
		switch (button.button) {
			case glam.Gamepad.TRIGGER_RIGHT : 
				if (button.pressed) {
					this.toggleStartStop();
				}
				break;
			case glam.Gamepad.BUTTON_A : 
				if (button.pressed) {
