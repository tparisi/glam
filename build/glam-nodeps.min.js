...
if(a["backface-visibility"])switch(a["backface-visibility"].toLowerCase()){case "visible":h=THREE.DoubleSide;break;case "hidden":h=THREE.FrontSide}var l;a.hasOwnProperty("render-mode")&&(l="wireframe"==a["render-mode"]);var p;a["line-width"]&&(p=parseInt(a["line-width"]));var q;a["dash-size"]&&(q=parseInt(a["dash-size"]));var u;a["gap-size"]&&(u=parseInt(a["gap-size"]));a={};var w=new THREE.TextureLoader;b.image&&(a.map=w.load(b.image));b.envMap&&(a.envMap=b.envMap);b.normalMap&&(a.normalMap=w.load(b.normalMap));
...
return 6==b.length?(a=new THREE.CubeTextureLoader,a.load(b,THREE.Texture.DEFAULT_MAPPING,function(a){})):a["sphere-image"]?(b=glam.DOMMaterial.parseUrl(a["sphere-image"]),a=new THREE.CubeTextureLoader,a.load(b,THREE.SphericalRefractionMapping,function(a){})):null};
