...
c=a.length;for(b=0;b<c;b++)$.parsecss(a[b].childNodes[0].data,function(a){glam.DOMDocument.addStyle(a)})}};glam.CSSParser={};
(function(a){function b(a){a=g[a].replace(/^{|}parser/g,"");a=c(a);var b={};a.split(";").forEach(function(a){a=a.split(":");2>a.length||(b[d(a[0])]=d(a.slice(1).join(":")))});return b}function c(a,b){var c,d;a=a.replace(m,"parser1").replace(k,function(a,b){if(!b)return"";var c="%s`"+ ++p+"`s%";g[p]=b.replace(/^\\/,"");return c});for(var e=b?h:l;c=e.exec(a);)d="%b`"+ ++p+"`b%",g[p]=c[0],a=a.replace(e,d);return a}function d(a){var b;if(void 0===a)return a;for(;b=t.exec(a);)a=a.replace(t,g[b[1]]);return a.trim()}
...
f+")"))}catch(g){b.push(f)}}return b};a.parsecss.styleAttributes=function(b,e){var f="",g,h={};b=b.replace(u,"").replace(w,"parser1");c(b).replace(x,function(a,b,c){b=b.toLowerCase();h[b]?++h[b]:h[b]=1;if(g=/\bstyle\s*=\s*(%s`\d+`s%)/i.exec(c))a=(a=/\bid\s*=\s*(\S+)/i.exec(c))?"#"+d(a[1]).replace(/^['"]|['"]parser/g,""):b+":eq("+(h[b]-1)+")",f+=[a,"{",d(g[1]).replace(/^['"]|['"]parser/g,""),"}"].join("")});a.parsecss(f,e)};var f={},g={},l=/{[^{}]*}/,h=/\[[^\[\]]*\]|{[^{}]*}|\([^()]*\)|function(\s+\w+)?(\s*%b`\d+`b%){2}/,
m=/\/\*@((?:[^\*]|\*[^\/])*)\*\//g,k=/(?:\/\*(?:[^\*]|\*[^\/])*\*\/)|(\\.|"(?:[^\\\"]|\\.|\\\n)*"|'(?:[^\\\']|\\.|\\\n)*')/g,t=/%\w`(\d+)`\w%/,p=0,u=/\x3c!--([^-]|-[^-])*--\x3e/g,w=/(>)[^<]*/g,x=/<(\w+)([^>]*)>/g})(glam.CSSParser);glam.AnimationElement={};glam.AnimationElement.DEFAULT_DURATION="1s";glam.AnimationElement.DEFAULT_ITERATION_COUNT="1";glam.AnimationElement.DEFAULT_TIMING_FUNCTION="linear";glam.AnimationElement.DEFAULT_FRAME_TIME="0%";glam.AnimationElement.DEFAULT_FRAME_PROPERTY="transform";
...
"helvetiker");e=e.toLowerCase();"bold"!=e&&(e="normal");f.toLowerCase();f="normal";g=parseFloat(g);l=parseFloat(l);m=parseFloat(m);k=parseFloat(k);b="bevel"==h.toLowerCase()?!0:!1;l||(b=!1);b||(k=m=glam.TextElement.BEVEL_EPSILON,b=!0);h=a.getAttribute("value")||glam.TextElement.DEFAULT_VALUE;h||(h=a.textContent);c.value=h;c.fontSize=g;c.fontDepth=l;c.bevelSize=m;c.bevelThickness=k;c.bevelEnabled=b;c.fontFamily=d;c.fontWeight=e;c.fontStyle=f};
