...
glam.Transition.create = function(docelt) {

	var id = docelt.id;
	var duration = docelt.getAttribute('duration') || glam.Transition.DEFAULT_DURATION;
	var iterationCount = docelt.getAttribute('iteration-count') || glam.Transition.DEFAULT_ITERATION_COUNT;
	var timingFunction = docelt.getAttribute('timing-function') || glam.Transition.DEFAULT_TIMING_FUNCTION;
...
glam.Transition.parse = function(docelt, obj) {
	var animationId = docelt.getAttribute('animation');
	if (animationId) {
		var animation = glam.getAnimation(animationId);
		if (animation) {
			glam.Transition.addAnimationToObject(animation, obj);
		}
		else {
			glam.Transition.addParseCallback(animationId, function(animation) {
				glam.Transition.addAnimationToObject(animation, obj);				
			});
		}
	}
