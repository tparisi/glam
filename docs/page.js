var onDocumentLoad = function ( event ) {
	...
	var section = /\/(manual|reference)\//.exec( pathname )[ 1 ].toString().split( '.html' )[ 0 ];
	...
	if ( section == 'manual' ) {
		...
		path = /\/reference\/[A-z0-9\/]+/.exec( pathname ).toString().substr( 11 );
	...
	text = text.replace(/\[(?:member|property|method|attribute):([\w]+)\]/gi, "[member:$1 $1]" ); // [member:name] to [member:name title]
	text = text.replace(/\[(?:member|property|method|attribute):([\w]+) ([\w\.\s]+)\]/gi, "<a href=\"javascript:window.parent.goTo('"+name+".$2')\" target=\"_parent\" title=\""+name+".$2\" class=\"permalink\">#</a><a href=\"javascript:window.parent.goTo('$1')\" title=\"$1\" id=\"$2\">$2</a> " );
